...
tweets_clean <- tweets %>%
  ...
  mutate(airline_sentiment_number = ifelse(airline_sentiment == "negative", -1,
                                           ifelse(airline_sentiment == "positive", 1, 0)),
         user_timezone = as.factor(user_timezone),
         user_location =
...
tweets_clean_aggregate <- tweets_clean %>% 
  group_by(user_location, airline) %>% 
